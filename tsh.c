...
/* Global variables */
volatile sig_atomic_t flag = -1; // child handler
...
void eval(char *cmdline) {
  ...
  int bg;
  pid_t pid;
  sigset_t mask_all, block_mask, prev_mask;
...
void waitfg(pid_t pid) {
  ...
  sigprocmask(SIG_BLOCK, &block_set, &prev);
  while (flag == pid) {
    sigsuspend(&prev);
  }
